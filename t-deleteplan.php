<?php include "inc/t-header.php"; ?>
  ...
  <div class="container-overlay">
    ...
    <h3 class="section-title">Form to Delete Plan</h3>
    ...
    <?php
        ...
        if(isset($_POST['delete'])){
          deleteWorkoutPlan($member_id, $workout_date);
        }
      ?>
      <form method="POST">
        <fieldset>
          <label for="workoutdate">Workout Date:</label>
          <input disabled type="date" id="workoutdate" name="workoutdate" value="<?php if(!empty($workout_date)) echo $workout_date; ?>">
        </fieldset>
        <fieldset>
          <label for="starttime">Start Time:</label>
          <input disabled type="time" id="starttime" name="starttime" value="<?php if(!empty($start_time)) echo $start_time; ?>">
        </fieldset>
        <fieldset>
          <label for="endtime">End Time:</label>
          <input disabled type="time" id="endtime" name="endtime" value="<?php if(!empty($end_time)) echo $end_time;?>">
        </fieldset>
        <fieldset>
          <label for="workouts">Workout Name</label>
          <select disabled id="workouts" name="workouts">
           <?php
